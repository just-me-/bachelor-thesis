...
The dependency of \code{extensions} on VSCode is based on dependency injection and represents the plugin entry point.
...
This has the following reason: Up until this refactoring, the task of \code{stringResources} was to be a central collection of all user interface (UI) strings. \glsadd{UI}
...
\code{LanguageServerInstaller} component.
...
Second, \code{serverInitializer} registers a callback as the program entry point to the created \linebreak
\code{serverOptions} instance and then the language server is started with \code{this.languageServer.start()}.
...
This includes the status bar information and the display of the \code{CounterModel}.
...
A \code{disposeUi} function is also provided, which destroys the UI elements. This is the case
on a Dafny language server restart. On restart, all UI components are rebuilt, \glsadd{UI}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{implementation/commands_shown}
...
\code{IDafnyRunner} to call compile and run as a callback as you can see in listing \ref{lst:compileAndRun}.
Furthermore, \code{IDafnyUiManager} is injected
in order to access the \code{CounterModelProvider} with the appropriate commands and to show or hide it.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{implementation/msg_example_warning}
    \caption{Warning Message Received From Server}
    \label{fig:msg_example}
\end{figure}

The \code{Notifications} component is responsible for displaying messages received from the server.
...
The \code{CodeLensProvider} component is very simple. \textit{CodeLens} data transferred from the server is transferred to the VSCode API.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{implementation/opened_codelens_exmpl}
    \caption{\textit{CodeLens} Opened After Clicking on References}
    \label{fig:opened_codelens_exmpl}
\end{figure}

The \code{CounterModelProvider} component is used to display and hide counter examples.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm]{implementation/dark_light_mode_2}
    \caption{CounterModel in Light (Top) and in Dark Mode (Bottom)}
    \label{fig:dark_light_mode_2}
\end{figure}

Unlike the \textit{Compile} feature, the \code{CounterModelProvider} creates a callback and passes it to the \linebreak
\code{serverRequest} module.
...
\code{CounterModelProvider} has an active influence on when exactly a request is sent.
On the other hand, the components for the status bar and \textit{CodeLens} hand react passively to an event.

This way the \code{CounterModel} component can regulate the server requests.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{implementation/dafny_statusbar}
...
For macOS and Linux these are executed using Mono. \glsadd{mono}
...
Under Windows, \code{.exe} files can be executed natively.
...
This component is only used by \code{Compile}.
...
Contrary to the design, this module does not only contain UI messages displayed to the user as mentioned in section \glsadd{UI}
...
we wrote an additional module \code{ideApi} as shown in figure \ref{fig:packageIdeApi}.
...
The idea here is that when migrating to another IDE, adapter components can be written in \code{ideApi}
for the VSCode modules used in a centralized way.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{implementation/module_ideApi}
    \caption{Extract of the Modules and Interfaces Implemented by the Module \code{ideApi}}
...
We did not analyze in detail in what languages plugins have to be developed for other editors.
...
either accept adjustments directly there, or he will be forwarded to \code{settings.json} as shown in figure \ref{fig:settings_vscode}.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{implementation/settings_vscode}
...
Just as in the chapter \nameref{chapter:design}, the server structure will be analyzed from top to bottom, starting with the \code{Main}-method and ending at the utility layer.
...
For example, every time the server receives a \code{textDocument/Definitions} request, that \code{Handle} method will be called.
The parameter and return types are specific per request.
\textit{GoToDefinition} would pass a text document location as input, namely the cursor position, and it expects a \code{LocationLink} as response,
...
All handlers require two additional methods, apart from the actual \code{Handle}:
\begin{itemize}
    \item \code{GetRegistrationOptions}: This method is called when the handler is registered.
    ...
    \item \code{SetCapability}: It allows to set handler-specific capabilities, such as if the \textit{Rename}-handler will also support a 'prepare rename' feature.
\end{itemize}
...
There are a few features that had to be extended with additional logic.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=9cm]{implementation/workspace.png}
    \caption{\code{WorkspaceManager} and \code{FileRepository}}
    \label{fig:worksapceAndRepo}
\end{figure}

Aside from the file content, each \code{FileRepository} will also contain \code{TranslationResults}. \linebreak
...
\begin{figure}[H]
    \centering
    \includegraphics[width=6cm]{implementation/DTUSeq.png}
    \caption{\code{DafnyTranslationUnit} Sequence Diagram}
    \label{fig:DTUSeq}
\end{figure}

In the constructor, the translation unit accepts a \code{PhysicalFile}.
The class then offers a single method \code{public TranslationResults Verify()}.
...
\begin{enumerate}
    ...
    \item If successful, the precompiled \code{DafnyProgram} will be split into \code{BoogiePrograms}.
    \item The Boogie execution engine is invoked to perform logical correctness checks on the \code{BoogiePrograms}.
    \item Any errors that were reported are collected, converted and assembled in the field \\
    \code{\_diagnosticElements}.
...
\code{Config/LanguageServerConfig.json} is parsed with Newtonsoft's Json.NET library \cite{jsondotnet}. \glsadd{json}
...
This is, for example, \code{method}, \code{class}, or \code{return}.
...
Technically, the symbol table is more of a double linked tree, than a table.
...
It can have children (the variable \code{x}), it has a parent, and it can be used elsewhere in the code.
...
The declaration of \code{x} in the second line will have many null values within the \code{SymbolInformation}.
...
\textbf{Visitors}\\
As already mentioned, the whole symbol table generation is realized using the visitor pattern. \glsadd{visitor}
...
Thus, the visitor has a boolean property \code{GoesDeep}, which decides whether method bodies are visited or not.
The final \code{Accept} method for a method looks as shown in listing \ref{lst:visitoraccept}.
The method is shortened, there are more clauses such as the \code{requires} clause.
...
This is the case since every AST-Element is either a statement, a definition or a declaration, among others. \glsadd{ast}
...
Let us continue the example of visiting a \code{method}.
...
\begin{itemize}
\item \code{TopDown}: Starting from a node, the navigator dives downwards and searches a specific symbol.
\item \code{BottomUp}: Starting from a node, the navigator climbs upwards the tree and searches a specific symbol.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{impl_symboltablenav}
    \caption{\code{TopDown} (Left) and \code{BottomUp} (Right) Visualized}
...
The algorithm will then decide, that the cursor position is within module \code{M}.
...
When checking class \code{C}, wrapping is successful, thus the next iteration will be done within \code{C}.
Finally, variable \code{c} will be returned as the best match.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=11.5cm]{implementation/topDownPaintFTW.png}
...
Lastly, clauses like \code{ensures} are in between those two.
Figure \ref{fig:topDownWraps} shows the distinction between the three methods.

\begin{figure}[H]
    \centering
    \includegraphics[width=8cm]{implementation/wraps.png}
...
With the help of the \code{SymbolTableNavigator},
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth/2]{get_users_desire}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{object_completion_diagram_dot}
...
the \code{CompletionProvider} first looks for an access symbol as entry point at the cursor position.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=12cm]{new_completion_diagram_classes}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=12cm]{default_completion_diagram_all}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{implementation/xcode}
...
Using \code{TextEdits}, the registered function would then jump to the next placeholder parameter
...
the property \code{Length} would then have to be suggested for the array \code{a}.

\begin{figure}[H]
    \centering
    \includegraphics[width=10cm]{implementation/array_support}
...
This allows the developer to efficiently jump between different places within a workspace. \glsadd{codelens}\\
...
To each \textit{CodeLens} element, a \code{Command} object is also appended.
...
If the function \code{dafny.showReferences} is called,
the method \code{VSCodeCommandStrings.ShowReferences} from the VSCode API is invoked
...
need to be set initially to achieve a \textit{CounterExample}.\\

The \textit{CounterExample} provider reads the model file and uses the Boogie backend to convert it into a useful format, which is called \code{ILanguageSpecificModel}.
...
Values providing no information at all are completely removed from the \textit{CounterExample}.
...
Mono allows to execute \code{.exe} files on said systems \cite{mono}. \glsadd{mono} \\
...
\begin{itemize}
    ...
    \item We tried to start a very simple language server - without the Dafny project - and could not execute it correctly with \code{msbuild}. \glsadd{msbuild}
...
First, the \code{DafnaTranslationUnit} is called to create a \code{DafnyProgram}.
The \code{DafnyProgram} is then handed to the \code{SymbolTableGenerator}.
...
Technically, since the \code{TranslationUnit} is invoked as well, the test is not completely isolated.
One would have to serialize the \code{DafnyProgram}, and use that one as a test input.
...
There is not a single test that checks if symbol \code{X} gets used 5 times - and checks only that.
...
It is not exactly tested, if symbol \code{X} is used by symbol \code{Y}.
...
To test \textit{CounterExample}, a custom \code{model.bvd} file can be injected into the provider.
This way, predefined model files are analyzed instead of the model from the live Dafny code.\\

To test \textit{Compile}, a \code{.dfy} file is defined in the test method.
...
\end{lstlisting}
The class providing these references will also check if the test file actually exists, so that \linebreak \code{FileNotFoundErrors} can be detected early.\\
...
\begin{lstlisting}[language=csharp, caption={Converting \textit{CounterExamples} to a List of Strings}, captionpos=b, label={lst:genericconverter}]
...
\code{CollectionAssert.AreEquivalent(expectation, actual)} from nUnit's test framework,
the two lists can be easily compared against each other \cite{nunitCollectionAssert}. \glsadd{nunit} \\
...
It inherits from the \code{IntegrationTestBase} class and provides the member \linebreak \code{CompilerResults},
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm]{integrationTestDesign.png}
...
for example in the case of \textit{AutoCompletion} or \textit{HoverInformation},
as a \code{documentation} element \cite{vscodeAPI}.

\subsubsection{Cleaner HoverInformation}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{implementation/hover_before_after}
...
You should not have to press \code{CTRL+Space} as trigger.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{implementation/cleaner_error_msg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=9cm]{implementation/runPython}
    \caption{Play Button to Run Python Code}
    \label{fig:runPython}
\end{figure}

It is possible to also add this icon for Dafny. If the button is clicked, \textit{CompileAndRun} is triggered.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{implementation/RunDafny}
...
Prettier was selected for this function \cite{dev}.
...
For an easier testability of the CI, we installed Docker locally. \glsadd{docker}
