...
In this way the CounterModel component can regulate the server requests.
...
Atom JS,
Eclipse java
Andere IDEs haben wir nicht in usnerer arbeit analysiert
daher haben wir nicht zu viel zeit für die isolation investiert
falls nötig können aber durch diese grobe isolation innerhalb dieses moduls adapter komponenten geschrieben werden .
falls eine ganz andere programmiersprache für eine andere IDE verwendet werden muss,
muss die logik der einzeonen komponenten entsprechend migriert werden und der aufwand steigt entsprechend.
neutraler name
...
zwei layers. einmal komplett vom user konfigurierbar, einmal in den stringressourcen.
darin sind beispielsweise Dafny file endungen und sonstige konfigurationen, die nicht direkt vom benutzer geändert werden wollen oder sollen.
...
Since not only CounterExamples, but also other data structures such as \code{Diagnostic} were converted into lists of strings, the converters were held generic as far as possible.
...
A list of CounterExamples is handed as the argument, and a \code{Func<CounterExample, string> ToCustomString} is handed as the converter.
...
The same applies for test about CounterExamples, GoToDefinition and other use cases.
