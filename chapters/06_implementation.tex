...
The dependency of \code{extensions} on VSCode is based on dependeny injection and represents the plugin entry point.
...
The CounterModuel supports a dark and light mode as shown in figure \ref{fig:dark_light_mode_2}.
...
\begin{figure}[H]
    ...
    \caption{Dafny Statusbar for a Invalid (Top) and a Valid File (Bottom)}
    \label{fig:statusbar}
\end{figure}
...
\end{itemize}
These challanges are discussed below. \\
...
Reagarding figure \ref{fig:fig:topDownPaintFTW}, imagine we hand the blue line as the cursor position.
...
The available statistics, like the finding of bugs as shown in fugure \ref{fig:sonarexample}, are very helpful for code reviews.\\
