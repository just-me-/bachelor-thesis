...
Just as in the chapter design, the server structure will be analyzed from top to bottom, starting from the \code{Main}-method and ending at the utility layer.
...
\begin{itemize}
    ...
    \item \code{SetCapability}: It allows to set handler-specific capabilities, such as if the rename-handler will also support a 'prepare rename' feature.
\end{itemize}
...
Aside from the file content, each \code{FileRepository} will also contain \code{TranslationResults}.
...
\begin{figure}[H]
    ...
    \caption{\code{DafnyTranslationUnit} Sequence}
    \label{fig:DTUSeq}
\end{figure}
...
Afterwards, the file \\
\code{Config/LanguageServerConfig.json} is parsed with Newtonsoft's Json.NET library \cite{jsondotnet}.
...
It can have children (the variable \code{x}), it has some parent, and it can be used elsewhere in the code.
...
When checking on class \code{C}, wrapping is successful, thus the next iteration will be done within \code{C}.
...
For clarity, the described process is visualized for the following
Dafny code example \ref{lst:myClassCompletionExampleUML} in the sequence diagram of figure \ref{fig:object_completion_diagram_dot}.
...
the method \\
\code{VSCodeCommandStrings.ShowReferences} from the VSCode API is called
...
\code{DafnaTranslationUnit} to create a \code{dafnyProgram}.
The dafnyProgram is then handed to the \\
\code{SymbolTableGenerator}.
...
\end{lstlisting}
 ...
 \intnote{dfy ist hier auch noch ein magic string}
...
It is possible to also add this icon for Dafny. If the button is clicked, a \textit{Compile and Run} is triggered.
