...
Some of these functions have already been implemented in a certain base implementation in our prototype.
But almost all these features need to be improved.
A more detailed extension of feature richness is discussed in the analysis part of this paper.
This shall be realized by the implementation of an custom symbol table. \\
...
Additionally, new features based on the symbol table will be added.
This includes especially the hover information.
...
This includes the recording of Sonarqube for the Dafyn Language Server as well as a clean logging,
which should be helpful for troubleshooting - especially for performance problems. \\

Furthermore, our plugin should reach a stable state and be published in the VS Code Market place.
So for Dafny developers at the end of our work an essential added value results from our thesis.
Therefore our plugin has to be platform independent and easy to install. \\

The IDE independent Dafny language server should get a merge back ready state.
This means that the server is extracted separately from the client into a separate git repository.
So at the end of the work a rebase of the Dafny server should be possible. 
