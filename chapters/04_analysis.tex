...
Finally, research done in relation to CI is described. \gls{ci}
...
\begin{itemize}
    ...
    \item \code{rename}
    \item \code{prepare\textit{Rename}}
...
\begin{itemize}
    ...
    \item CounterExample: The client sends a file identifier to the server and receives
...
public class \textit{AutoCompletion} : ICompletionHandler
{
...
For \textit{AutoCompletion}, this is the file and the cursor position, as well as some context information, how the \textit{AutoCompletion} event was triggered.
...
As we see now, the second \code{a} is a pseudo-variable, or how Dafny calls it, a \textit{AutoGhostIdentifier}.\\

\code{ComprehensionExpression} contains a variety of subclasses, such as the \code{ForallExpr}, \code{SetComprehension} and \code{LambdaExpr}.
...
\subsubsection{\textit{AutoCompletion} for Identifiers}
...
\begin{figure}[H]
    ...
    \caption{\textit{AutoCompletion} for Instance of Class C}
...
That means if the \textit{AutoCompletion} is called after a dot, the word before the dot is extracted from the source code.
...
the \textit{AutoCompletion} would no longer work properly. \\
...
\subsubsection{\textit{GoToDefinition}}
