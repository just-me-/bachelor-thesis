...
\begin{figure}[h]
    ...
    \caption{Communication Benefit of LSP, https://code.visualstudio.com/api/language-extensions/language-server-extension-guide} \intnote{muss man das bild iwie speziell reffen? - so abstract mässig? Image from YX [Ref]}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{04_lsplanguagefeatures.png}
...
Custom LSP messages are further discussed in the chapter \ref{chapter:customlspmsg}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{analysis/vs_code_extension_file_structure}
...
To be able to implement the required feature set for this project, the following questions must be answered by the symbol table or the AST, respectively.
 ...
 \intnote{Why do you need this information.}
...
Screenshot \ref{fig:namesegment} shows all available properties and fields of a name segment.
...
\subsection{Analysis of the Existing Dafny Visitor Component}
 \intnote{In diesem Kapitel nur die Analyse "was hatte Dafny bisher, warum unbrauchbar"}
  \intnote{Was der Visitor ist und wie wir den implementieren und warum genau kommt im design}
...
\begin{itemize}
    \item NameSegment: Any name of a variable or method.  \intnote{Monospace for class names}
    \item IdentifierExpression: A single variable.
    \item ExprDotName: An expression containing a dot as a locator, such as \code{myclass.mymethod()}.
    ...
    \item UnaryOpExpression: A unary expression with an Operator, mostly connected to the "not"-Operator, for example \code{!someBoolean}.
    \item ITEExpr: If-then-else expression, such as \code{if a < 0 then -a else a}
    \item ThisExpr: the \code{this} referring to the own class instance.
    \item ParensExpression: Any expression surrounded by brackets.
    \item ChainingExpression: Short syntax with a chain of operands like \code{a < b < c < d}.
...
\subsubsection{Statements}
 \intnote{Laborious, but can you give a code example for every node?}
...
\begin{itemize}
    ...
    \item PrintStmt: A print statement such as \code{print a}.
    \item ProduceStmt: This node occurs at return values.
    \item VarDeclStmt: A variable declaration.
    \item UpdateStmt: An update statement, such as \code{a = 2}.
...
\begin{itemize}
   ...
   \item TypeRhs: A right hand side containing a type, such as \code{new MyClass()}.
...
\intnote{ ich hab zwei anmerkungen zu diesem kapitel:}
...
\intnote{anmerkungen end}

\intnote{anmerkungen antwort}
1. so ist nun mal der aufbau von ner wissenschaftlichen arbeit, erst mal ziele definieren,w as man erreichen will. dann analyse des ist zustands und ggf was man daran noch verbessern muss. bei der implementation tut man dann beschreiben wie man dieses ziel erreicht hat. und was man erreicht hat. und beim result tut dann dass erreichte nochmals grob zusammen fassen und mit dem ursprünglichen ziel/einleitung/geplanten vergleichen - und ausblicke setzen von nicht erreichtem oder was man noch erreichen könnte.
...
Dafny works with so called \code{ErrorReporter}s, that should provide the necessary information.\\
...
\intnote{todo quellen auf die funktionen von der vs code api}
