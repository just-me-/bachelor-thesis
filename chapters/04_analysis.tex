...
sections \ref{section:analysis:lsp} and \ref{section:analysis:omnisharp} are repeated from the semester thesis.\\ \glsadd{omnisharp}
...
Finally, research done in relation to CI is described. \glsadd{ci}

\subsection{Language Server Protocol}
\label{section:analysis:lsp}
The language server protocol (LSP) is a JSON-RPC based protocol to communicate between an IDE and a language server \cite{lspWiki}. \glsadd{jsonrpc}
...
\begin{itemize}
    ...
    \item \code{completion/resolve}
    \item \code{hover}
    \item \code{vsignatureHelp}
    \item \code{declaration}
    \item \code{definition}
    \item \code{typeDefinition}
    \item \code{implementation}
    \item \code{references}
    \item \code{documentHighlight}
    \item \code{documentSymbol}
    \item \code{codeAction}
    \item \code{codeLens}
    \item \code{codeLens/resolve}
    \item \code{documentLink}
    \item \code{documentLink/resolve}
    \item \code{documentColor}
    \item \code{colorPresentation}
    \item \code{formatting}
    \item \code{rangeFormatting}
    \item \code{onTypeFormatting}
    \item \code{rename}
    \item \code{prepareRename}
    \item \code{foldingRange}
    \item \code{selectionRange}
\end{itemize}
\end{multicols}
...
The message contains the URI of the opened document, so that the language server can load the source code. \glsadd{uri}
...
Mr. Martin Bj\"orkstr\"om published a comprehensible tutorial about OmniSharp's language server protocol implementation \cite{omnisharptutorial}. \glsadd{omnisharp}
...
The current problem domain does not only require premade LSP messages like \code{completion} or \linebreak
\code{publishDiagnostics}, but also custom requests such as \code{CounterExample}, which is Dafny-specific.
...
\begin{lstlisting}[language=csharp, caption={LSP Handler Implementation for \textit{CounterExample}}, captionpos=b, label={lst:lspcustomhandler}]
...
\begin{lstlisting}[language=typescript, caption={\code{extension.ts}}, captionpos=b, label={lst:extension-file}]
...
Since the Dafny language server is not part of the plugin delivery to the VSCode marketplace,
...
\begin{itemize}
    ...
    Afterwards, they may type \code{H.addOne(5)}.
...
the keyword \code{this} needs to write in front of the variable name.
...
\code{this.a} will still yield \code{2}, even inside the nested scope.\\
...
The right hand side is a binary expression, with the \code{+}-operator in between two operands.
...
The colors just represent different branches of the tree for clarity.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=8cm]{analysis/namesegmentbsp.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=8cm]{analysis/methodbsp.png}
...
\begin{itemize}
    ...
    \item \code{Declaration}
\end{itemize}
Besides these, some AST-nodes are isolated, such as \code{AssignmentRHS}, which is the right hand side of an assignment.
...
For example, a \linebreak \code{IdentifierExpr} is nothing more than containing a \code{NameSegment}.\\
...
Internally, this declaration is represented as \linebreak \code{var a : int; a := 2}.
As we see now, the second \code{a} is a pseudo-variable, or how Dafny calls it, a \code{AutoGhostIdentifier}.\\

\code{ComprehensionExpression} contains a variety of subclasses, such as the \code{ForallExpr}, \linebreak \code{SetComprehension} and \code{LambdaExpr}.
All of these contain bound variables (the parameters of the lambda for example) and an expression afterwards.\\

\code{|a|} denotes the size of the set \code{a}.\\
...
During analysis of the Dafny AST, it was noticed that the file \code{DafnyAst.cs} is overwhelmingly large.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm]{analysis/syntax_highlighting}
...
Among other things, new keywords are now recognized and visually marked, such as \code{expect}.
...
This includes syntax errors. If the compiled program can be run, a PowerShell instance is started directly inside Visual Studio Code. \glsadd{powershell}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=12cm]{analysis/counter_example}
...
\begin{figure}[H]
    ...
    \caption{\textit{AutoCompletion} for Instance of Class \code{C}}
...
In addition, it would be very useful for users if \textit{AutoCompletion} is triggered after a \code{new}
...
\begin{figure}[H]
    ...
    \caption{\textit{GoToDefinition} via Context Menu}
...
As an example, the class \code{C} has been instantiated twice and the method \code{m} was used once in the following code snippet in figure \ref{fig:analysis_codelens}. \glsadd{codelens}
...
\begin{figure}[H]
    ...
    \caption{\textit{CodeLens} Example}
...
On the server side, the project was automatically built by the CI server using \textit{MSBuild}.  \glsadd{msbuild}
Unit tests were executed using the \textit{nUnit} console runner \cite{nunit}. \glsadd{nunit} \glsadd{lit}
...
Excluded from this principle are \textit{Node}, \textit{Z3}, \textit{Go}, \textit{Boogie} and \textit{Sonar}.  \glsadd{z3} \glsadd{boogie} \glsadd{sonar}
