...
The representation is much cleaner and easier to immerse. \intnote{todo dieses untere bild updaten. verification streicht neu alles an.}
...
\begin{figure}[H]
    ...
    \caption{\textit{Rename} of a Symbol}
    \label{fig:rename}
\end{figure}
...
\textit{CodeLens} is an excellent way for the developer to see which classes and methods were used how often - and where.
...
foreach (var symbol in fileSymboltable.GetFullList())
{
  ...
  // create \textit{CodeLens} with symbolReferenceCounter
}
\end{lstlisting}
...
Since the \code{textDocuemt/didOpen}, which triggers the symbol table instantiation, is not awaitable, a random LSP request was sent after the opening to wait until all actions are finished.
...
For comparison: the original language server written in TypeScript had 3'100 LOC, rounded off \cite{sonar-old_ts_server}.
