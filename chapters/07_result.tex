...
If the targeted file is not opened, VS Code will just open it inside the workspace.
...
\intnote{ich habe mir erlaubt, zu kürzen, weil das 1:1 in implementaiton stand mit der unterscheidung was wann wie vorgeschlagen wird.}
\intnote{habe versucht die 3 fälle in einem bild zusammenzufassen.}
...
There is also still a problem with platform independence as described in in chapter \ref{section:implementation:mono} \nameref{section:implementation:mono}.
...
\intnote{das war nicht so verständlich hier. hab versucht es zu clarifyen. ist noch alles richtig was hier steht?}
...
The artifacts could also be made available through the CI process as a static website with GitLab Pages \cite{gitlab-pages}. \intnote{zu technisch für result?}
...
\subsection{Metrics adn Quality Measures}
...
Compared to the prototype, this is an increase of about 250\%.
...
Intersting is the comparison of the code base required for a feature like GoToDefinition.
...
The Sonar report for the server only detects 15 issues.
...
For example, Sonar would like us to put all \code{Visit} and all \code{Leave} methods together.
However, we prefer to have the corresponding \code{Visit} and \code{Leave} for an AST node together.
