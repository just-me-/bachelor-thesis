...
{\bf StringResources} \textendash{} \intnote{resource mit einem s - evtl im code auch noch korrigieren}
...
{\bf Server Connection} \textendash{} \intnote{eig wär hier ein custom command, so subsubsubchaptermässig angezeigt. aber ich bin zu faul :P}
...
\subsection{Server}
\intnote{noch ne übersicht iwo, wie figure 9 SA - todo}
\intnote{iuntro todo, mimi, heir übersicht, gehen top down durch im folgenden}\\
...
\begin{figure}[H]
    \centering
    \includegraphics[width=12.5cm]{serverdesign/01_basic_idea}
...
\intnote{todo bild. FileRepo mit was halt da drin is. und workspace und alles.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=12.5cm]{serverdesign/02_DTU.png}
    \caption{Dafny Translation Unit}
    \label{fig:server_dtu}
\end{figure}
\intnote{ist die lesbarkeit nur bei mir so schlecht der skalierten grafiken? habs mal etwas grösser gemacht.}
\intnote{toto evtl doch noch den converter bei commons reinfügen der die diagnostics rumwuselt (evtl)}


Excluded from the figure are some converter methods, that convert \code{Diagnostics}, such as warnings and errors, to an uniform format.
...
Most often, this is the \code{FileRepository}, which the handler requested from the \code{WorkspaceManager}.
...
\intnote{bild, alle kapitel 1 bild. }
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm]{serverdesign/resources.png}
