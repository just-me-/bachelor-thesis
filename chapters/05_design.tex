...
\intnote{eher mit gros H und pagebreak arbeiten für grafiken, ist ausser kontrolle mit kleinen h's.}
...
One distinguishes whether the execution of \code{.exe} files should be done with \textit{Mono} (on macOS and Linux operating systems) or not. \\
...
Since both, \code{Handler}s and \code{Provider}s require these, they were extracted to a separate package.
...
Finally, to have a link to the symbol table, a reference to a \code{SymbolTableManager} was added.\\

Since \code{TranslationResults} and \code{PhysicalFile} are used in multiple parts of the code, these classes were place within a \code{Commons} package.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=7.5cm]{design/server7_resources.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=7.5cm]{design/SYMBOLTABLE.png}
...
since there is no longer just 'one' global space, but one per file, as long as they don't include each other.\\
