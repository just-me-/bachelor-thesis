...
The handlers themselves will have to implement a specific interface by OmniSharp, for example \code{I\textit{Rename}Handler}.
...
There is one class for each feature, for example \code{\textit{Rename}Provider} or \code{DiagnsoticsProvider}.
...
Figure \ref{fig:server_core} shows the core package with the example of the \code{\textit{Rename}Provider}.
...
\begin{figure}[H]
    ...
    \caption{Sequence Diagram of a \textit{Rename} Request}
    \label{fig:workflow1}
\end{figure}
...
\begin{itemize}
    \item Find a symbol at the cursor position and go to symbol declarations for the \textit{GoToDefinition} feature.
...
\begin{itemize}
    \item If a file gets included elsewhere - the file is not aware about this. \textit{CodeLens} and \textit{Rename} will not work properly.
...
An alternative solution to make features like \textit{Rename} and \textit{GoToDefinition} know about where they are included is to keep a table.
