...
This component is the aforementioned \code{main} of the plugin and serves as an entry point.
...
All types, in particular the types created specifically for additional functions such as \code{CounterExampleResults}, were defined by interfaces. \\
...
\intnote{OmniSharp Omnisharp, VSCode VS Code. überall einheitlich. ctrl+h am ende... das bringt nix da jetzt anzufangne.}
...
These interfaces demand a \code{Handle} method with the proper arguments and results.
...
The class \code{PhysicalFile} just represents a file on the user's filesystem and will be discussed in the next chapter.
The only public method \code{Verify} will start the Dafny verification process based on this file
...
Excluded from the figure are some converter methods, that convert \code{Diagnstoics}, such as warnings and errors, to an uniform format.
...
Since the user is editing the code, it also provides an \code{Update} method.
...
It contains a dictionary, linking a file identificator (an URI) to a \code{FileRepository}.
...
The manager will forward the call towards the \code{FileRepository}, which will adjust the \code{PhysicalFile}, but also generate new \code{TranslationResults} and \textit{a new symbol table!!} by invoking the Dafny Access package. Any changes are then stored in its \code{Buffer} property.
...
Furthermore, the \code{dafnyProgram} is available from the \code{WorkspaceManager} for every file.
...
The \code{SymbolInformation} class is supposed to contain any necessary data about every symbol.
...
The \code{Navigator} is responsible to navigate through the symbol table.
...
The \code{Manager} is responsible to build the table.
It will already make use of the navigator, namely to locate declarations of symbols.

All three components are highly coupled.
...
Most often, this is the \code{FileRepository}, which the handler requested from the WorkspaceManager.
...
For example, if we want to test if the compiler reports a missing semicolon, we could create a testclass \code{public class SyntaxErrors : CompileBase}.
Note that we inherit from hour case-specific base class.
Thus, the methods \code{RunCompilation} and\code{Verify} are at our dispoal.
